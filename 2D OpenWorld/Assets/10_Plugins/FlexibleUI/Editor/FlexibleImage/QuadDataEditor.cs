...
public class QuadDataEditor
{
    ...
    private static GUIStyle _paleBoxStyle;
    private static GUIStyle PaleBoxStyle => _paleBoxStyle ??= new (GUI.skin.box) {normal = {background = EditorGUIUtility.whiteTexture}, alignment = TextAnchor.MiddleCenter};
    ...
    private static readonly GUIContent OutlineAccommodatesCollapsedEdgeContent = new ("Massage Collapse", "Enlarges the image so that corners of the outline are not cut off when an edge is partially/fully collapsed. This affects vertex color gradients by pushing them outside the border perimeter.");
    ...
    public (bool verticesDirty, bool raycastAreaDirty) DrawQuadDataEditor (SerializedObject serializedObject, float originalLabelWidth, float originalFieldWidth, FlexibleImage flexibleImage = null, bool useAdvancedRaycast = false, SerializedProperty spriteProperty = null, GUIStyle sectionStyleOverride = null)
    {
        ...
        if (SessionState.GetBool(ShowColorSectionKey, false))
        {
            ...
            controlSection = showProceduralGradientSection ? (0, 0f, 9999f) : (0f, 45f, 9999f);
            lastSection = (hasColorPreset, showProceduralGradientSection) switch
            {
                (true, false) => (presetMixLabelWidth, 25f, 35f),
                (_, true ) => (alphaIsBlendLabelWidth, 18f, 18f),
                _ => (0f, 0f, 0f)
            ...
            else
            {
                ...
                if (proceduralGradientArea == SecondaryColorArea.Outline && currentProps.outlineWidthProperty.floatValue <= 0f)
                ...
                if (proceduralGradientTypeProperty.enumValueIndex == (int)QuadData.GradientType.SDF)
                {
                    ...
                    GUI.enabled = true;
                    if (proceduralGradientPositionFromPointerProperty.boolValue)
                ...
                else
                {
                    ...
                    if (proceduralGradientPositionFromPointerProperty.boolValue && !screenSpaceProceduralGradientProperty.boolValue && proceduralGradientTypeProperty.enumValueIndex != (int)QuadData.GradientType.SDF && (meshSubdivisionsProperty.intValue > 2 || meshSubdivisionsProperty.intValue == 2 && meshTopologyProperty.enumValueIndex == (int)QuadData.Topology.X))
                ...
                if (proceduralGradientPositionFromPointerProperty.boolValue && !Application.isPlaying && flexibleImage != null)
                {
            ...
            controlSection = showPatternSection ? (0, 0f, 9999f) : (0f, 45f, 9999f);
            lastSection = (hasColorPreset, showPatternSection) switch
            {
                (true, false) => (presetMixLabelWidth, 25f, 35f),
                (_, true ) => (alphaIsBlendLabelWidth, 18f, 18f),
                _ => (0f, 0f, 0f)
            ...
            else
            {
                ...
                if (patternArea == SecondaryColorArea.Outline && currentProps.outlineWidthProperty.floatValue <= 0f)
                    EditorGUILayout.HelpBox("Set to outline region, but outline width is 0.", MessageType.Warning);
                ...
                else
                {
                    ...
                    if (blurEnabled)
                    {
                        ...
                        EditorGUI.LabelField(controlRect, controlRect.width > 175 ? PatternSpriteRotationUnavailableContent : PatternSpriteRotationUnavailableContentShort, PaleBoxStyle);
                    }
                    else
                    {
                        GUI.enabled = isDefaultProps;
                ...
                void HandlePatternLineThickness(GUIContent content)
                {
                    ...
                    if (blurEnabled)
                    {
                        ...
                        controlRect.x += labelOffset;
                        controlRect.width -= labelOffset;
                        EditorGUI.LabelField(controlRect, controlRect.width > 175 ? PatternLineThicknessUnavailableContent : PatternLineThicknessUnavailableContentShort, PaleBoxStyle);
                    }
                    else
                    {
        ...
        if (shapeSectionExpanded)
        {
            ...
            if (SessionState.GetBool(ShowCornersKey, false))
            {
                ...
                if (mirrorCollapseProperty.boolValue)
                {
                    ...
                    controlRect.x += labelOffset;
                    controlRect.width -= labelOffset;
                    EditorGUI.LabelField(controlRect, controlRect.width > 182.5f ? NormalizeChamferUnavailableContent : NormalizeChamferUnavailableContentShort, PaleBoxStyle);
                }
                else
                {
                    GUI.enabled = isDefaultProps;
            ...
            else
            {
                ...
                var chamfer = currentProps.cornerChamferProperty.vector4Value;
                var uniformChamfer = Mathf.Approximately(chamfer.x, chamfer.y) && Mathf.Approximately(chamfer.x, chamfer.z) && Mathf.Approximately(chamfer.x, chamfer.w);
                var mixedChamfer = !uniformChamfer || currentProps.cornerChamferProperty.hasMultipleDifferentValues;
                
                EditorGUI.showMixedValue = mixedChamfer;
                EditorGUI.BeginChangeCheck();
                DrawDetentedFloatFieldWithDefault(rectArr[0], defaultProps.cornerXChamferProperty, currentProps.cornerXChamferProperty, CornerChamferContent, 65536, 0f, 4095.9375f, shrinkField: true, useFloatFieldMethods: true);
                if (EditorGUI.EndChangeCheck())
                    currentProps.cornerChamferProperty.vector4Value = Vector4.one * currentProps.cornerXChamferProperty.floatValue;

                var concavity = currentProps.cornerConcavityProperty.vector4Value;
                var uniformConcavity = Mathf.Approximately(concavity.x, concavity.y) && Mathf.Approximately(concavity.x, concavity.z) && Mathf.Approximately(concavity.x, concavity.w);
                var mixedConcavity = !uniformConcavity || currentProps.cornerConcavityProperty.hasMultipleDifferentValues;

                EditorGUI.showMixedValue = mixedConcavity;
                EditorGUI.BeginChangeCheck();
                DrawDetentedFloatFieldWithDefault(rectArr[1], defaultProps.cornerXConcavityProperty, currentProps.cornerXConcavityProperty, concavityIsSmoothingProperty.boolValue ? CornerSmoothingContent : CornerConcavityContent, 256, 0f, concavityIsSmoothingProperty.boolValue ? 1f : 1.9921875f, shrinkField: true, useFloatFieldMethods: true);
                if (EditorGUI.EndChangeCheck())
                    currentProps.cornerConcavityProperty.vector4Value = Vector4.one * currentProps.cornerXConcavityProperty.floatValue;
    ...
    private static void DrawColorGrid(SerializedObject serializedObject, SerializedProperty dimensionsProp, SerializedProperty wrapModeXProp, SerializedProperty wrapModeYProp, SerializedProperty defaultGridProp, SerializedProperty currentGridProp, SerializedProperty defaultOffsetProp, SerializedProperty currentOffsetProp, SerializedProperty defaultRotationProp, SerializedProperty currentRotationProp,  SerializedProperty defaultScaleProp, SerializedProperty currentScaleProp, bool isDefaultProps, string advancedOptionsKey)
    {
        ...
        EditorGUILayout.BeginVertical(PaleBoxStyle);
        EditorGUILayout.Space(0.5f);
