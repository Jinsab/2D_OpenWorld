...
public class FlexibleBlurFeature : ScriptableRendererFeature 
{
...
#if UNITY_EDITOR
    public static readonly string BlursHandledFieldName = nameof(blurTypesHandled);
    ...
    public static readonly Dictionary<Camera, List<IBlur>> ImageBasedBlurDict = new();
    public static readonly Dictionary<Camera, int> ImageBasedLayersPerCameraDict = new();
    ...
    public static readonly Dictionary<Camera, FlexibleBlurPass> GlobalFlexibleBlurPassDict = new();
    public static readonly Dictionary<(Camera camera, int layer), RTHandle> NewLayerHandles = new();
    public static readonly List<Shader> RegisteredImageShaders = new();
    public static readonly Dictionary<(Shader shader, Camera camera, int layer), Material> NewMaterials = new();

    [Flags] public enum BlurTypesHandled { UIBlur = 1, BlurredImage = 2}
    [SerializeField] private BlurTypesHandled blurTypesHandled = (BlurTypesHandled)3;
    ...
    public override void Create()
    {
        ...
        pass = new(blurTypesHandled, renderPassEvent, (UnityEngine.FilterMode)destinationFilterMode, useComputeShaders, resultFormat, blurFormat, uiBlurLayersSeeLower, blurredImagesSeeUIBlurs, blurredImageLayersSeeLower, overlayCompatibilityFix, maxLayerResolution, layerResolutionRatio);
    }
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        {
            if (GlobalFlexibleBlurPassDict.TryGetValue(layerKvp.Key.camera, out var blurPass))
...
public partial class FlexibleBlurPass : ScriptableRenderPass
{
    ...
    private static readonly Dictionary<Camera, List<RTHandle>> globalRTHandleDict = new();
    private static readonly Dictionary<Shader, Dictionary<Camera, List<Material>>> globalImageMatDict = new();
    ...
    private readonly Dictionary<Camera, List<RTHandle>> instanceLayerRTHandleDict = new();
    private readonly Dictionary<Shader, Dictionary<Camera, List<Material>>> instanceImageMatDict = new();
    private readonly PooledListDictionary<(BlurSettings blurSettings, float alpha), List<IBlur>, IBlur> batchedBlurs = new ();
    private readonly FlexibleBlurFeature.BlurTypesHandled blurTypesHandled;
    private readonly float layerResolutionRatio;
    private readonly int maxLayerResolution;
    ...
    public static bool TryGetImageMaterial(Camera camera, int layer, Shader shader, out Material mat)
    {
        if (!globalImageMatDict.TryGetValue(shader, out var innerGlobalImageMatDict))
        {
            innerGlobalImageMatDict = globalImageMatDict[shader] = new Dictionary<Camera, List<Material>>();
            if (!FlexibleBlurFeature.RegisteredImageShaders.Contains(shader))
                FlexibleBlurFeature.RegisteredImageShaders.Add(shader);
        }

        if (innerGlobalImageMatDict.TryGetValue(camera, out var matList))
        {
        ...
        var key = (shader, camera, layer);
        if (FlexibleBlurFeature.NewMaterials.TryGetValue(key, out mat))
            return true;

        if (!FlexibleBlurFeature.GlobalFlexibleBlurPassDict.TryGetValue(camera, out _))
            return false;
    ...
    public static bool TryGetImageRT (Camera camera, int layer, out RTHandle handle)
    {
        if (globalRTHandleDict.TryGetValue(camera, out var rtList))
        {
        ...
        var key = (camera, layer);
        if (FlexibleBlurFeature.NewLayerHandles.TryGetValue(key, out handle))
            return true;

        if (!FlexibleBlurFeature.GlobalFlexibleBlurPassDict.TryGetValue(camera, out var pass))
            return false;
    ...
    public FlexibleBlurPass(FlexibleBlurFeature.BlurTypesHandled blurTypesHandled, RenderPassEvent renderPassEvent, FilterMode destinationFilterMode, bool useComputeShaders, GraphicsFormat resultGraphicsFormat, GraphicsFormat blurGraphicsFormat, bool uiBlurLayersSeeLower, bool blurredImageSeeUIBlurs, bool blurredImageLayersSeeLower, bool overlayCompatibilityFix, int maxLayerResolution, float layerResolutionRatio)
    {
        InstanceHandleSystem = new();

        (this.destinationFilterMode, this.useComputeShaders, this.resultGraphicsFormat, this.blurGraphicsFormat, this.uiBlurLayersSeeLower, this.blurredImageSeeUIBlurs, this.blurredImageLayersSeeLower, this.overlayCompatibilityFix, this.maxLayerResolution, this.layerResolutionRatio) = 
        (     destinationFilterMode,      useComputeShaders,      resultGraphicsFormat,      blurGraphicsFormat,      uiBlurLayersSeeLower,      blurredImageSeeUIBlurs,      blurredImageLayersSeeLower,      overlayCompatibilityFix,      maxLayerResolution,      layerResolutionRatio);

        this.blurTypesHandled = blurTypesHandled;
    ...
    private void Setup(Camera camera, RenderTextureDescriptor descriptor)
    {
        ...
        if (blurTypesHandled.HasFlag(FlexibleBlurFeature.BlurTypesHandled.UIBlur))
        {
            UIBlur.LayersPerCameraDict.TryGetValue(currentCamera, out var uiBlurLayers);
            if (uiBlurLayers > 0)
        ...
        if (blurTypesHandled.HasFlag(FlexibleBlurFeature.BlurTypesHandled.BlurredImage))
        {
            FlexibleBlurFeature.ImageBasedLayersPerCameraDict.TryGetValue(currentCamera, out var texturesNeededForImageBlurs);

            numLayerTextures = Math.Max(texturesNeededForUiBlurs, texturesNeededForImageBlurs);

            if (!instanceLayerRTHandleDict.TryGetValue(currentCamera, out currentRTHandleList))
                currentRTHandleList = instanceLayerRTHandleDict[currentCamera] = new List<RTHandle>();

            while (currentRTHandleList.Count < numLayerTextures)
            {
                blurLayerAdddedThisFrame = true;
                var newHandleKey = (camera, currentRTHandleList.Count);
            ...
            globalRTHandleDict[currentCamera] = currentRTHandleList;

            foreach (var shader in FlexibleBlurFeature.RegisteredImageShaders)
            {
                if (!instanceImageMatDict.TryGetValue(shader, out var innerImageMatDict))
                    innerImageMatDict = instanceImageMatDict[shader] = new Dictionary<Camera, List<Material>>();

                if (!innerImageMatDict.TryGetValue(currentCamera, out var matList))
                    matList = innerImageMatDict[currentCamera] = new List<Material>();

                while (matList.Count < texturesNeededForImageBlurs)
                {
                    var newMaterialKey = (shader, camera, matList.Count);
                ...
                innerGlobalImageMatDict[currentCamera] = matList;
            }
        }
        ...
        FlexibleBlurFeature.GlobalFlexibleBlurPassDict[camera] = this;
    }
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        UIBlur.BlurDict.TryGetValue(camera, out var blurAreas);
        FlexibleBlurFeature.ImageBasedBlurDict.TryGetValue(camera, out var blurredImageAreas);

        var haveUIBlurAreas = blurTypesHandled.HasFlag(FlexibleBlurFeature.BlurTypesHandled.UIBlur) && blurAreas is { Count: > 0 };
        var haveBlurredImageAreas = blurTypesHandled.HasFlag(FlexibleBlurFeature.BlurTypesHandled.BlurredImage) && blurredImageAreas is { Count: > 0 };
        ...
        void HandleBlurredImages()
        {
            ...
            int numImageLayers = FlexibleBlurFeature.ImageBasedLayersPerCameraDict[camera];
            var source = cameraRT;
