...
public partial class FlexibleBlurPass
{
    ...
    public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
    {
        ...
        (passData.camera, passData.renderScale, passData.originalWidth, passData.originalHeight, passData.singlePassVR, passData.haveUIBlurAreas, passData.haveBlurredImageAreas, passData.blurAreas, passData.blurredImageAreas, passData.batchedBlurs, passData.blurredImageSeeUIBlurs, passData.blurredImageLayersSeeLower, passData.uiBlurLayersSeeLower, passData.useComputeShaders, passData.overlayCompatibilityFix) = 
        (         camera,          renderScale,          originalWidth,          originalHeight,          singlePassVR,          haveUIBlurAreas,          haveBlurredImageAreas,          blurAreas,          blurredImageAreas,          batchedBlurs,          blurredImageSeeUIBlurs,          blurredImageLayersSeeLower,          uiBlurLayersSeeLower,          useComputeShaders,          overlayCompatibilityFix);
        ...
        for (var i = 0; i < instanceLayerRTHandleDict[camera].Count; i++)
        {
            var texture = renderGraph.ImportTexture(instanceLayerRTHandleDict[camera][i]);
    ...
    private static void Pass(BlurPassData data, UnsafeGraphContext context)
    {
        ...
        var (textureHandle, textureDimensions, textureIndices, camera, renderScale, originalWidth, originalHeight, singlePassVR, multiPassVR, rightEye, haveUIBlurAreas, haveBlurredImageAreas, blurAreas, blurredImageAreas, batchedBlurs, blurredImageLayersSeeLower, blurredImageSeeUIBlurs, uiBlurLayersSeeLower, useComputeShaders, overlayCompatibilityFix) =
            (data.textureHandles, data.textureDimensions, data.textureIndices, data.camera, data.renderScale, data.originalWidth, data.originalHeight, data.singlePassVR, data.multiPassVR, data.rightEye, data.haveUIBlurAreas, data.haveBlurredImageAreas, data.blurAreas, data.blurredImageAreas,data.batchedBlurs, data.blurredImageLayersSeeLower, data.blurredImageSeeUIBlurs, data.uiBlurLayersSeeLower, data.useComputeShaders, data.overlayCompatibilityFix);
        ...
        void HandleBlurredImages()
        {
            ...
            var numImageLayers = FlexibleBlurFeature.ImageBasedLayersPerCameraDict[camera];
