...
public class UIBlurCommon
{
    ...
    private IBlur blur;
    private Dictionary<Camera, List<IBlur>> blurDict;
    private Dictionary<Camera, int> layersPerCameraDict;
    ...
    private bool cachedUseFilterPadding, cachedFitRotatedImageWithinBounds;

    public void Init(IBlur blur, Dictionary<Camera, List<IBlur>> blurDict, Dictionary<Camera, int> layersPerCamera) => (this.blur, this.blurDict, this.layersPerCameraDict) = (blur, blurDict, layersPerCamera);
    ...
    public void PlaceInBlurList(Camera camera)
    {
        if (blur is UIBlur && blur.Alpha == 0 && !blur.ActiveAtZeroAlpha)
            return;

        if (!blurDict.TryGetValue(camera, out var blurList))
            blurList = blurDict[camera] = new List<IBlur>();

        if (blurList.Count == 0)
        {
            if (!layersPerCameraDict.TryAdd(camera, 1))
                layersPerCameraDict[camera] += 1;
    ...
    public void RemoveFromBlurList()
    {
        ...
        if (!PresentInBlurList || prevCamera == null || !blurDict.TryGetValue(prevCamera, out var blurList))
        {
            prevCamera = null;
        ...
        if (blurList.Count == 0)
            blurDict.Remove(prevCamera);
        else for (int i = idx; i < blurList.Count; i++)
            blurList[i].Common.LayerRank--;

        var cameraLayers = --layersPerCameraDict[prevCamera];
        if (cameraLayers == 0)
            layersPerCameraDict.Remove(prevCamera);

        prevCamera = null;
        PresentInBlurList = false;
    }

    public Camera GetCameraReference() => cameraReference ? cameraReference : Camera.main;
    ...
    public void CacheBlur(Canvas canvas, RectTransform rectTransform, Vector2 offset, float rotation, bool fitRotatedImageWithinBounds, Vector2 shapePadding, float blurPadding  = 0f, bool useFilterPadding = true)
    {
        (cachedCanvas, cachedRectTransform, cachedOffset, cachedRotation, cachedFitRotatedImageWithinBounds, cachedShapePadding, cachedBlurPadding, cachedUseFilterPadding) = (canvas, rectTransform, offset, rotation, fitRotatedImageWithinBounds, shapePadding, blurPadding, useFilterPadding);
        hasCachedBlur = true;
    }
    ...
    public void ComputeBlur(float filteringPadding)
    {
        ...
        ComputeBlurCommon(cachedCanvas, cachedRectTransform, cachedOffset, cachedRotation, cachedFitRotatedImageWithinBounds, cachedShapePadding, cachedBlurPadding, cachedUseFilterPadding ? filteringPadding : 0f);
        hasCachedBlur = false;
    }

    public void ComputeBlurCommon(Canvas canvas, RectTransform rectTransform, Vector2 offset, float rotation, bool fitRotationInsideOriginalBounds, Vector2 shapePadding, float blurPadding = 0f, float filteringPadding = 0f)
    {
        var cameraKey = GetCameraReference();
        if (!cameraKey)
        {
            RemoveFromBlurList();
            return;
        ...
        if (canvas.renderMode == RenderMode.ScreenSpaceCamera)
            ...
            WorldCamera = canvas.worldCamera ?? cameraKey;
        else
            WorldCamera = null;
        ...
        if (PresentInBlurList && (cameraKey != prevCamera || priority != prevPriority || unrankedLayer != prevUnrankedLayer))
        {
            RemoveFromBlurList();
            PlaceInBlurList(cameraKey);
        }

        var canvasCamera = WorldCamera ?? cameraKey;
