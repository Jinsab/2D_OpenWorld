Shader "Hidden/JeffGrawAssets/ProceduralBlurredImage"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #define IsSimpleSDF(v)                      (bool)(v.flagsAndEdgeCollapseIdx.x & (1 << 28))
            ...
            v2f vert(const appdata_t v)
            {
                ...
                [branch] if (PatternParam2(OUT) > 0)
                {
                    ...
                    else if (PatternType(OUT) == 24) // fractals
                    {
                    ...
                    else if (PatternType(OUT) == 25) // sprite
                    {
            ...
            half4 frag(v2f IN) : SV_Target
            {
                ...
                [branch] if (HasProceduralGradient(IN))
                {
                    ...
                    [branch] if (IsSDFOrNoiseGradient(IN))
                    {
                        ...
                        else // noise gradient
                        {
                            ...
                            lerpVal = saturate(n + IN.proceduralGradientParams.w);
                        }
                ...
                color.a *= saturate(visible) * (CutoutOnlyAffectsOutline(IN) ? 1 : cutoutResult);
