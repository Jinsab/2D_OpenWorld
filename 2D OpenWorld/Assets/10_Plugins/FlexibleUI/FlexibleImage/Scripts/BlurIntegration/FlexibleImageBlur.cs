...
public partial class FlexibleImage : IBlur
{
    ...
    partial void GetMaterialForRenderingBlur(ref Material mat)
    {
        ...
        var cameraKey = Common.GetCameraReference();
        if (!cameraKey)
            return;

        FlexibleBlurPass.TryGetImageRT(cameraKey, Common.LayerRank, out var newHandle);
        if (newHandle == null)
        {
            mat = null;
            return; 
    ...
    partial void LateUpdateBlur()
    {
        ...
        var cameraKey = Common.GetCameraReference();
        if (!cameraKey || !FlexibleBlurPass.TryGetImageMaterial(cameraKey, Common.LayerRank, ImageShader, out var mat))
            return;
    ...
    partial void CalculateBlur()
    {
        ...
        Common.CacheBlur(canvas, rectTransform, offset, rotation, quadData.FitRotatedImageWithinBounds, sizeModifier * canvas.scaleFactor, additionalBlurPadding);
        if (lastComponentEnabledFrame != Time.frameCount)
            Common.ComputeBlurCommon(canvas, rectTransform, offset, rotation, quadData.FitRotatedImageWithinBounds, sizeModifier * canvas.scaleFactor, additionalBlurPadding);
    }
}
}
