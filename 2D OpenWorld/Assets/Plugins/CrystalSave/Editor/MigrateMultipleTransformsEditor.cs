...
{
	[CustomEditor(typeof(MigrateMultipleTransforms))]
	public class MigrateMultipleTransformsEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.LabelField("Transform Migration Entries", EditorStyles.boldLabel);
			EditorGUILayout.HelpBox(
				"For each entry, assign a Source GameObject that has both a UniqueID and a RememberTransform component. " +
			...
			for (int i = 0; i < migrationEntriesProp.arraySize; i++)
			{
				...
				if (entryProp.isExpanded)
				{
					...
					// If no valid unique ID is set, default to showing the source field.
					...
					if (editSourceFlags[i])
					{
						...
						if (newSource != currentSource)
						{
							if (newSource != null)
							{
								// Check for required components: UniqueID and RememberTransform.
								UniqueID uniqueIDComp = newSource.GetComponent<UniqueID>();
								RememberTransform rememberTransformComp = newSource.GetComponent<RememberTransform>();
								if (uniqueIDComp == null || rememberTransformComp == null)
								{
									EditorUtility.DisplayDialog("Missing Component",
										$"The selected GameObject '{newSource.name}' must have both a UniqueID and a RememberTransform component.",
										"OK");
									// Reset the source.
									...
									editSourceFlags[i] = true; // Stay in edit mode.
								}
								else
								{
									sourceObjects[i] = newSource;
									string computedID = $"{uniqueIDComp.ID}_{rememberTransformComp.ComponentID}";
					...
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newPosition"), new GUIContent("New Position"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newEulerRotation"), new GUIContent("New Euler Rotation"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newScale"), new GUIContent("New Scale"));
			...
			if (GUILayout.Button("Add New Transform Migration Entry"))
			{
