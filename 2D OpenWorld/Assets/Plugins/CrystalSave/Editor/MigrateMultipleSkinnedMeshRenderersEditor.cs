...
{
	[CustomEditor(typeof(MigrateMultipleSkinnedMeshRenderers))]
	public class MigrateMultipleSkinnedMeshRenderersEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			for (int i = 0; i < migrationEntriesProp.arraySize; i++)
			{
				...
				// Get targetName for the foldout label.
				...
				if (entryProp.isExpanded)
				{
					...
					// Show the Source field if no valid targetUniqueID is set.
					...
					if (editSourceFlags[i])
					{
						...
						if (newSource != currentSource)
						{
							if (newSource != null)
							{
								// Check that the GameObject has both UniqueID and RememberMeshRenderer components.
								...
								if (uniqueIDComp == null || rememberMeshComp == null)
								{
									EditorUtility.DisplayDialog("Missing Component",
										...
										"OK");
									// Reset the source so it remains empty.
					...
					DrawConditionalProperty(entryProp, "updateSharedMaterials", "newSharedMaterialNames", "New Shared Material Paths");
					DrawConditionalProperty(entryProp, "updateUniqueMaterialProperties", "newUniqueMaterialProperties", "New Unique Material Data");
					DrawConditionalProperty(entryProp, "updateBlendShapeWeights", "newBlendShapeWeights", "New Blend Shape Weights");
					...
					DrawConditionalProperty(entryProp, "updateRootBone", "newRootBoneName", "New Root Bone Name");
					DrawConditionalProperty(entryProp, "updateQuality", "newQuality", "New Skin Quality");
					DrawConditionalProperty(entryProp, "updateUpdateWhenOffscreen", "newUpdateWhenOffscreen", "New Update When Offscreen");
