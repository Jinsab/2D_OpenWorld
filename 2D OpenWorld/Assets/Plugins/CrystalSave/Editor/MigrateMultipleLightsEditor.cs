...
{
	[CustomEditor(typeof(MigrateMultipleLights))]
	public class MigrateMultipleLightsEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			EditorGUILayout.LabelField("Light Migration Entries", EditorStyles.boldLabel);
			EditorGUILayout.HelpBox(
				"For each entry, assign a Source GameObject that has both a UniqueID and a RememberLight component. " +
			...
			for (int i = 0; i < migrationEntriesProp.arraySize; i++)
			{
				...
				// Get the targetName for the foldout label.
				...
				if (entryProp.isExpanded)
				{
					...
					// Show Source field if targetUniqueID is empty.
					...
					if (editSourceFlags[i])
					{
						...
						if (newSource != currentSource)
						{
							if (newSource != null)
							{
								// Check for required components: UniqueID and RememberLight.
								UniqueID uniqueIDComp = newSource.GetComponent<UniqueID>();
								RememberLight rememberLightComp = newSource.GetComponent<RememberLight>();
								if (uniqueIDComp == null || rememberLightComp == null)
								{
									EditorUtility.DisplayDialog("Missing Component",
										$"The selected GameObject '{newSource.name}' must have both a UniqueID and a RememberLight component.",
										"OK");
								...
								else
								{
									...
									string computedID = $"{uniqueIDComp.ID}_{rememberLightComp.ComponentID}";
					...
					EditorGUILayout.Space();
					EditorGUILayout.LabelField("Light Property Updates", EditorStyles.boldLabel);
					// Display property fields for new values.
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newEnabled"), new GUIContent("New Enabled"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newColor"), new GUIContent("New Color"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newIntensity"), new GUIContent("New Intensity"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newShadowsEnabled"), new GUIContent("New Shadows Enabled"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newRange"), new GUIContent("New Range"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newSpotAngle"), new GUIContent("New Spot Angle"));
					EditorGUILayout.PropertyField(entryProp.FindPropertyRelative("newSize"), new GUIContent("New Size"));
